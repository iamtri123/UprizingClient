...
public class GuiWaypointsMenu extends GuiScreen implements GuiYesNoCallback { // TODO: Optimize
    ...
    public void confirmClicked(boolean par1, int par2) {
        if (yesNoClicked) {
            if (par1) {
                uprizing.getWaypoints().deleteWaypoint(selectedWaypoint);
                selectedWaypoint = null;
            }
    ...
    class List extends GuiSlot {
        ...
        protected int getSize() {
            return uprizing.getWaypoints().getRenderer().count();
        }

        @Override
        protected void elementClicked(int par1, boolean par2, int par3, int par4) {
            setSelectedWaypoint(uprizing.getWaypoints().getRenderer().get(par1));
            ...
            if (mouseX >= leftEdge + width - 16 - padding && mouseX <= leftEdge + width + padding) {
                ...
                uprizing.getWaypoints().saveWaypoints();
            } else if (par2) {
                Mouse.next();
        ...
        protected boolean isSelected(int par1) {
            return selectedWaypoint == uprizing.getWaypoints().getRenderer().get(par1);
        }
        ...
        protected void drawSlot(int par1, int par2, int par3, int par4, Tessellator tessellator, int par6, int par7) {
            final Waypoint waypoint = uprizing.getWaypoints().getRenderer().get(par1);
