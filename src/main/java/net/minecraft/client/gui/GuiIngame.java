...
public class GuiIngame extends Gui {
    ...
    public void renderGameOverlay(float p_73830_1_, boolean p_73830_2_, int p_73830_3_, int p_73830_4_) {
        ...
        if (!this.mc.thePlayer.isPotionActive(Potion.confusion)) {
            ...
            if (var10 > 0.0F) {
                this.func_130015_b(var10, width, height);
            }
        }
        ...
        if (mc.gameSettings.showDebugInfo) {
            ...
            String var20 = "Used memory: " + usedMemory * 100L / maxMemory + "% (" + usedMemory / 1024L / 1024L + " MB) of " + maxMemory / 1024L / 1024L + " MB";
            this.drawString(var8, var20, width - var8.getStringWidth(var20) - 2, 2, 14737632);

            var20 = "Allocated memory: " + totalMemory * 100L / maxMemory + "% (" + totalMemory / 1024L / 1024L + " MB)";
            ...
            this.drawString(var8, String.format("X: %.5f (%d) ||| Chunk X: %d (%d)", this.mc.thePlayer.posX, var22, var22 >> 4, var22 & 15), 2, 64, 14737632);
            this.drawString(var8, String.format("Y: %.3f (feet pos), %.3f (eyes pos)", mc.thePlayer.boundingBox.minY, mc.thePlayer.posY), 2, 72, 14737632);
            this.drawString(var8, String.format("Z: %.5f (%d) ||| Chunk Z: %d (%d)", this.mc.thePlayer.posZ, var24, var24 >> 4, var24 & 15), 2, 80, 14737632);

            final int directionIndex = MathHelper.floor_double((double) (mc.thePlayer.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3;
            this.drawString(var8, "Direction (" + directionIndex + "): " + Direction.directions[directionIndex], 2, 88 + 8, 14737632);
            this.drawString(var8, "Yaw: " + MathHelper.wrapAngleTo180_float(mc.thePlayer.rotationYaw), 2, 88 + 16, 14737632);
            this.drawString(var8, "Real Yaw: " + mc.thePlayer.rotationYaw, 2, 88 + 24, 14737632);
            this.drawString(var8, "Pitch: " + mc.thePlayer.rotationPitch, 2, 88 + 32, 14737632);

            if (mc.theWorld != null && mc.theWorld.blockExists(var22, var23, var24)) {
                Chunk chunk = mc.theWorld.getChunkFromBlockCoords(var22, var24);
                this.drawString(var8, "LC: " + (chunk.getTopFilledSegment() + 15) + ", Biome: " + chunk.getBiomeGenForWorldCoords(var22 & 15, var24 & 15, this.mc.theWorld.getWorldChunkManager()).biomeName + ", BL: " + chunk.getSavedLightValue(EnumSkyBlock.Block, var22 & 15, var23, var24 & 15) + ", SL: " + chunk.getSavedLightValue(EnumSkyBlock.Sky, var22 & 15, var23, var24 & 15) + ", RL: " + chunk.getBlockLightValue(var22 & 15, var23, var24 & 15, 0), 2, 96 + 40, 14737632);
            }

            this.drawString(var8, String.format("Walk Speed: %.3f ||| Fly Speed: %.3f ||| On Ground: %b ||| Height Value: %d", mc.thePlayer.capabilities.getWalkSpeed(), mc.thePlayer.capabilities.getFlySpeed(), mc.thePlayer.onGround, mc.theWorld.getHeightValue(var22, var24)), 2, 104 + 40, 14737632);

            if (mc.entityRenderer != null && mc.entityRenderer.isShaderActive()) {
                drawString(var8, String.format("shader: %s", this.mc.entityRenderer.getShaderGroup().getShaderGroupName()), 2, 112 + 40, 14737632);
            }
        ...
        if (var37 != null) {
            this.mc.uprizing.getSidebarDrawer().draw(var37, height, width, var8);
        }
