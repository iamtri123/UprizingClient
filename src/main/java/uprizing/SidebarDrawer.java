...
public class SidebarDrawer extends Gui {
	...
	public final void draw(ScoreObjective objective, int scaledHeight, int scaledWidth, FontRenderer fontRenderer) {
		...
		if (scores.size() <= 15) {
			...
			final boolean scoreboardScores = uprizing.getBoolean(UprizingSettings.SCOREBOARD_SCORES);
			...
			final boolean flag = uprizing.getBoolean(UprizingSettings.SCOREBOARD_TEXT_SHADOW);
